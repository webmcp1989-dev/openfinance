...
- New exception-to-cash migrations are AR `202608300002` through `202608300006` and AP `202608300001` through `202608300007`. They add only application-owned records; no cross-database integration or shared credential was introduced. The later hardening migrations enforce PDF integrity and serialize idempotent remittance, inquiry, and exception-response retries.
...
- Migration `202608300001_bind_oauth_tokens_to_mcp.sql` is applied and the hook is enabled in the live AR project. A real DCR + PKCE consent flow produced an exact-audience token, all eight remote tools passed live calls, OAuth writes carried the client ID in AR audit events, reset returned `403`, grant revocation invalidated refresh, and the temporary client and credentials were removed.
