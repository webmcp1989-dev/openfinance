...
- AR migration `202608290007_repair_renderable_invoice_pdfs.sql` owns complete synthetic PDF generation. It repairs the four core fixtures and existing ERP imports, then uses a private insert trigger for future `ERP-*` invoices. Do not reintroduce header/EOF-only placeholders; Adobe and standards-aware parsers reject them.
...
- AR migration `202608290007_repair_renderable_invoice_pdfs.sql` is applied to the live AR project and its 12-assertion rollback-only suite passed. A repaired live download also passed strict `pypdf`, Poppler metadata/text checks, SHA-256 verification, and page rendering.
