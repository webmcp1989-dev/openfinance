...
- AP exception responses validate the current supplier, stored owner/action, actionable exception state, bounded message, exact requested evidence kind, canonical PDF payload, decoded size, and SHA-256 before committing the response and evidence. Only then may the same transaction resolve that exception and accept a disputed invoice, and only when no other actionable exception remains.
...
- A corrected AP revision cannot silently replace an AR receipt. `record_portal_result` requires the exact current reference in `supersedesPortalReference`; the database row-locks the invoice and rejects stale, missing, mixed, or implicit replacement writes.
- The one no-replacement AR acceptance transition is narrowly constrained to a current disputed `missing_delivery_proof` invoice, its unchanged portal reference, an actual tenant-owned stored proof-of-delivery document, and AP's exact accepted result. It clears the exception and records the evidence name in the same transaction. All other status changes continue through the general state machine.
- Buyer cases are not copied through a hidden service connection. After AP returns a case, the agent records an exact, strictly parsed case outcome into AR using the existing tenant-scoped delivery-event transaction. Each application remains authoritative only for its own ledger.
...
- AP document-mutation routes additionally require the internal `X-OpenFinance-Document-Approval` identifier. A five-minute tenant/user/action/idempotency-bound approval is prepared without PDF bytes, explicitly approved in the portal, and compared in PostgreSQL with a manifest independently derived from the final request. Missing, denied, expired, cross-user, cross-tenant, reused-with-different-content, or payload-mismatched approval fails closed. Approval assertion, business mutation, and single-use consumption are one transaction; exact consumed retries may only return the existing idempotent result. Legacy and private mutation signatures are not executable by authenticated clients. Client timeout or agent cancellation dismisses the pending panel without attempting to convert an already expired record into a decision; the database remains authoritative for expiry.
...
- Invoice PDFs are limited to 1 MB decoded and about 1.4 MB encoded, must use the accepted classic PDF profile (header, catalog, page, terminal `startxref` pointing to `xref`, and EOF), and must match their declared SHA-256.
- The AP browser accepts an empty OS-provided MIME type only for a `.pdf` filename so valid downloads remain portable across browsers. Its structural file check is UX-only: the backend independently enforces declared media type, canonical bytes, classic PDF structure, size, and checksum.
...
Read tools are accurately annotated. Before an invoice PDF crosses from OpenFinance to Acme for read-only validation, the caller must show the destination and exact candidate invoices, POs, and amounts and obtain informed transfer approval. The AP portal then independently blocks its three document-writing tools on an exact, backend-enforced human approval panel immediately before submission, evidence response, or corrected replacement. Browser cancellation, denial, dismissal, or expiry produces no document mutation. The UI remains fully usable and shows receipts, balance changes, and recent tenant-scoped database audit events for verification. WebMCP reads containing invoice, document, purchase-order, validation, or receipt data set `untrustedContentHint`; AP document validation rejects non-canonical base64 encodings before checking the bounded classic PDF structure, byte limit, and SHA-256 so the same bytes cannot have multiple accepted wire representations.
...
AP exception ownership is enforced at the database boundary. Supplier responses are accepted only for `supplier_ar` or shared exceptions that explicitly allow `respond_to_exception`; named required evidence must be present and pass the authoritative PDF contract before it can resolve the blocker. A `buyer_receiving`, `buyer_procurement`, or `buyer_ap` blocker cannot be answered or approved as though the supplier resolved it and must use a separately approved tracked inquiry. Opening that case deliberately leaves both the exception and invoice blocked.
