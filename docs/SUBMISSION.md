...
Successful tool calls immediately update both visible applications. Acme shows committed portal references and reduced PO balances; OpenFinance shows delivery outcomes, the unresolved exception, and a tenant-scoped audit trail. Seeded exception work makes authority visible: supplier AR can attach exact delivery proof, while a missing receipt belongs to buyer receiving. The agent says “This isn't mine to fix,” offers a tracked case, and cannot bypass that boundary through the AP backend. Failures return actionable business explanations rather than generic automation errors.
