...
7. It shows `INV-10482`, `INV-10491`, their POs and amounts, the $25,670 total, and the excluded invoice. The agent calls `submit_invoice_batch`; Acme's mandatory approval panel independently shows the exact destination, invoices, POs, amounts, total, PDF filenames, and hashes.
8. The human checks the exact portal manifest and chooses **Approve and submit**. Only then does the paused tool commit the two invoices. Approval and the existing idempotency key are bound to that exact payload and consumed in the same transaction.
...
- Exception evidence response and corrected invoice replacement open the same backend-enforced document approval panel. Buyer inquiries keep their existing exact human confirmation but do not enter the document gate because they submit no document.
