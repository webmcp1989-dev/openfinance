...
- `supabase/migrations/202609010001_align_narrated_demo_state.sql` preserves the every-second settlement rule while making the first invoice in the canonical approved pair deterministically payable with `PAY-20260830-0DD9D23B`. It also exposes the tenant-derived open-buyer-case UI read model without adding a WebMCP capability.
