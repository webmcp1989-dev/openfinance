...
create function private.escape_pdf_literal(p_value text)
returns text
language sql
immutable
strict
set search_path = ''
as $$
  select replace(replace(replace(p_value, E'\\', E'\\\\'), '(', E'\\('), ')', E'\\)');
$$;

revoke execute on function private.escape_pdf_literal(text) from public, anon, authenticated;

create function private.pdf_text_line(
  p_x integer,
  p_y integer,
  p_size integer,
  p_font text,
  p_value text,
  p_rgb text default '0.16 0.20 0.25'
)
returns text
language plpgsql
immutable
...
as $$
begin
  if p_font not in ('F1', 'F2') or p_size not between 6 and 36
     or p_rgb !~ '^[0-9.]+ [0-9.]+ [0-9.]+$' then
    raise exception using errcode = '22023', message = 'Invalid PDF text style';
  end if;
  return p_rgb || ' rg' || chr(10) ||
    'BT /' || p_font || ' ' || p_size || ' Tf 1 0 0 1 ' || p_x || ' ' || p_y ||
    ' Tm (' || private.escape_pdf_literal(p_value) || ') Tj ET' || chr(10);
end;
$$;

revoke execute on function private.pdf_text_line(integer, integer, integer, text, text, text)
  from public, anon, authenticated;

create function private.render_synthetic_invoice_pdf(
...
begin
  ...
  v_stream :=
    ...
    private.pdf_text_line(487, 341, 11, 'F2', v_amount, '1 1 1') ||
...
create or replace function private.ensure_renderable_erp_invoice_pdf()
returns trigger
language plpgsql
security definer
set search_path = ''
...
begin
  if new.invoice_number like 'ERP-%' then
    select organization.name into v_supplier_name
    ...
    if v_supplier_name is null or v_customer_name is null then
      raise exception using errcode = '23503', message = 'Invoice supplier or customer was not found';
    end if;
    v_document_bytes := private.render_synthetic_invoice_pdf(
      new.invoice_number, new.invoice_date, new.amount_minor, new.currency,
      new.purchase_order_number, v_supplier_name, v_customer_name
    );
...
drop function private.render_synthetic_invoice_pdf(text);
