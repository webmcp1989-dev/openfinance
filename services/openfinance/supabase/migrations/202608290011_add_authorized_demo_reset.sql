...
begin
  ...
  if v_updated_sync_state <> 1 then
    raise exception using errcode = 'P0002', message = 'Demo ERP state is incomplete';
  end if;
  ...
  if v_updated_invoices <> 4 then
    raise exception using errcode = 'P0002', message = 'Demo invoice baseline is incomplete';
  end if;

  insert into public.audit_events (
    organization_id, actor_user_id, action, entity_type, entity_id, details, created_at
  ) values (
    v_organization_id, v_user_id, 'demo_state_reset', 'organization',
