...
-- Keep the existing portal-result implementation as the normal-path engine.
-- The guarded wrapper below handles only explicit replacement receipts, where
-- the caller proves which current reference the new AP revision supersedes.
alter function private.record_delivery_event(
  public.delivery_event_type, text, text, jsonb
) rename to record_delivery_event_without_replacements;

revoke execute on function private.record_delivery_event_without_replacements(
  public.delivery_event_type, text, text, jsonb
...
declare
  ...
  v_result jsonb;
begin
  -- Invalid and ordinary requests remain governed by the already-hardened
  -- implementation. Only a portal-result item with the explicit concurrency
  -- token enters the replacement path.
